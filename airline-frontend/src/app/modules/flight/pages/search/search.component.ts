      ...
      <div class="container-app hero-content">
        <div class="hero-text animate-fade-in-up">
          ...
          <p class="hero-subtitle">
            200+ destinations · Best fares guaranteed · Free cancellation
          </p>
        </div>

        <!-- Search Card -->
        <div
          class="search-card animate-fade-in-up"
                        ...
                        <div class="pax-row">
                          ...
                          <div class="pax-counter">
                            <button
                              type="button"
                              (click)="
                                $event.stopPropagation(); decrement(type.key)
                              "
                            >
                              −
                            </button>
                            ...
                            <button
                              type="button"
                              (click)="
                                $event.stopPropagation(); increment(type.key)
                              "
                            >
                              +
                            </button>
        ...
        <div
          class="popular-routes animate-fade-in-up stagger"
...
export class SearchComponent implements OnInit {
  ...
  ngOnInit() {
    this.form.get('departureDate')?.setValue(this.today);
  }
  ...
  onOriginSearch(e: Event) {
    ...
    this.flight.getAirports(val).subscribe({
      next: (r) => this.originSuggestions.set(r),
      error: () => {},
    });
  }

  hideSuggestions() {
    // A 200ms delay gives the user's mouse click enough time to trigger selectDest()
    setTimeout(() => {
      this.originSuggestions.set([]);
  ...
  onDestSearch(e: Event) {
    ...
    this.flight
      .getAirports(val)
      .subscribe({ next: (r) => this.destSuggestions.set(r), error: () => {} });
  }
  ...
  togglePassengerPanel() {
    this.passengerPanelOpen.update((v) => !v);
  }
