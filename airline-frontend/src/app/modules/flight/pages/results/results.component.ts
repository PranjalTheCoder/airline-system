      ...
      <div class="container-app results-layout">
          ...
          <div class="filter-group">
            ...
            @for (slot of timeSlots; track slot.label) {
              <label class="filter-check">
                <input
                  type="checkbox"
                  (change)="toggleTimeSlot(slot, $any($event.target).checked)"
                />
              ...
              @for (airline of airlines(); track airline) {
                <label class="filter-check">
                  <input
                    type="checkbox"
                    (change)="
                      toggleAirline(airline, $any($event.target).checked)
                    "
                  />
                  <span>{{ airline }}</span>
                </label>
        ...
        <div class="results-main">
          ...
          <div class="results-header">
            <div class="results-summary">
              @if (!loading()) {
                ...
                <p>
                  {{ searchParams()?.departureDate | date: 'EEEE, d MMMM' }} ·
                  {{ searchParams()?.adults }} Adult(s) ·
                  {{ searchParams()?.cabinClass | titlecase }}
                </p>
              } @else {
                <div
                  class="skeleton"
                  style="height:28px;width:280px;margin-bottom:8px"
                ></div>
          ...
          @if (!loading()) {
            ...
            @for (
              flight of filteredFlights();
              track flight.id;
              let i = $index
            ) {
              <div
                class="flight-card card card-hover animate-fade-in-up"
              ...
              >
                ...
                <div class="fc-footer">
                  <div class="fc-pricing">
                    @for (
                      cabin of getAvailableCabins(flight);
                      track cabin.type
                    ) {
                      <div
                        class="cabin-price"
                      ...
                      >
                        <div class="cp-label">
                          {{ formatCabinLabel(cabin.type) }}
                        </div>
                        <div class="cp-price">
                          {{ cabin.currency }}
                          {{ cabin.basePrice | number: '1.0-0' }}
                        </div>
                        <div class="cp-seats">
                    ...
                    <div class="selected-price">
                      @if (getSelectedCabin(flight)) {
                        <span class="price-amount">
                          ...
                          {{
                            getSelectedCabin(flight)!.basePrice
                              | number: '1.0-0'
                          }}
                        </span>
...
export class ResultsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // ✅ FIX 1: Sync UI selected cabin with the exact parameter the user searched for
    this.selectedCabin = (params.cabinClass || 'ECONOMY').toUpperCase();
  ...
  calcPriceRange(flights: Flight[]) {
    ...
    if (prices.length > 0) {
      ...
      // ✅ FIX 2: Ensure maxPrice strictly initializes to maxFare so flights aren't hidden
  ...
  applyFilters() {
    ...
    results = results.filter((f) => {
      const cabin = f.cabinClasses.find(
        (c) => c.type.toUpperCase() === this.selectedCabin.toUpperCase(),
      );
      // ✅ FIX 3: Force strict Number parsing to prevent Javascript string-comparison bugs ("3500" <= "0")
      const price = cabin ? Number(cabin.basePrice) : 0;
      const max = Number(this.maxPrice);
      return cabin && price <= max;
    });
    ...
    results.sort((a, b) => {
      if (this.sortBy === 'price') {
        const pa =
          a.cabinClasses.find((c) => c.type === this.selectedCabin)
            ?.basePrice ?? 0;
        const pb =
          b.cabinClasses.find((c) => c.type === this.selectedCabin)
            ?.basePrice ?? 0;
  ...
  toggleTimeSlot(_: any, __: boolean) {
    this.applyFilters();
  }
  ...
  getSelectedCabin(flight: Flight): CabinClass | undefined {
    return (
      flight.cabinClasses.find((c) => c.type === this.selectedCabin) ??
  ...
  selectFlight(flight: Flight) {
    this.store.dispatch(
      BookingActions.selectOutboundFlight({
        flight,
